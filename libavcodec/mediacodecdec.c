...
static av_cold int mediacodec_decode_init(AVCodecContext *avctx)
{
    ...
    av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_decode_init finish! %d", ret);
    return ret;
}

static int mediacodec_receive_frame(AVCodecContext *avctx, AVFrame *frame)
{
    av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame start");
    MediaCodecH264DecContext *s = avctx->priv_data;
    int ret;
    ssize_t index;
    ...
    while (1) {
        if (s->ctx->current_input_buffer < 0) {
            ...
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame try to get input buffer");
            index = ff_AMediaCodec_dequeueInputBuffer(s->ctx->codec, 10000);
            if (index < 0) {
                get_input_buffer_retry++;
                av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame can't get input buffer %d,retry: %d", index, get_input_buffer_retry);
            ...
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame got input buffer %d", index);
        }

        /* try to flush any buffered packet data */
        if (s->buffered_pkt.size > 0) {
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame current packet is not empty, try to send to decoder");
        ...
        av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame try to get packet AVCodecContext");
        ret = ff_decode_get_packet(avctx, &s->buffered_pkt);
        if (ret == AVERROR_EOF) {
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame try to get packet got AVERROR_EOF");
            AVPacket null_pkt = { 0 };
            ...
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame try to get packet got EAGAIN and current input buffer:%d", s->ctx->current_input_buffer);
            return ff_mediacodec_dec_receive(avctx, s->ctx, frame, true);
        } else if (ret < 0) {
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame try to get packet got RET: %d", ret);
            return ret;
        } else {
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX mediacodec_receive_frame got AVPacket %p", &s->buffered_pkt);
        }
    }
