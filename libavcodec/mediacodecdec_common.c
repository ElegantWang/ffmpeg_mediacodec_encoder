...
static const struct {
    ...
    { COLOR_FormatYUV422PackedPlanar,                        AV_PIX_FMT_NV12    },
...
{
    av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_send start");
    int offset = 0;
    int need_draining = 0;
    uint8_t *data;
    ...
    int get_input_buffer_retry = 0;
    while (offset < pkt->size || (need_draining && !s->draining)) {
        if (index < 0) {
            index = ff_AMediaCodec_dequeueInputBuffer(codec, input_dequeue_timeout_us);
            if (ff_AMediaCodec_infoTryAgainLater(codec, index)) {
                av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_send No input buffer available, try again later\n");
                break;
            }

            if (index < 0) {
                av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_send Failed to dequeue input buffer (status=%zd) retry:%d\n", index, get_input_buffer_retry);
                get_input_buffer_retry++;
                if(get_input_buffer_retry < 3) {
                    continue;
                } else {
                    return AVERROR_EXTERNAL;
                }
        ...
        av_log(avctx, AV_LOG_ERROR,
               "XXXXXXXXXX Queued input buffer %zd size=%zd ts=%"PRIi64"\n", index, size, pts);
    }
...
int save_frame(char *filename, AVFrame *frame) {
    FILE *yuvf;
    int i,j,w;
    uint8_t *pos;

    yuvf = fopen(filename, "wb+");
    if (!yuvf) {
        return -1;
    }
    pos = frame->data[0];
    ...
    for(i = 0; i < frame->height; i++) {
        fwrite(pos, sizeof(uint8_t), w, yuvf);
        pos += frame->linesize[0];
    }
    w = FFMIN(frame->linesize[1], FFALIGN(w, 2));
    pos = frame->data[1];
...
{
    av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive start");
    ...
    if (index >= 0) {
        av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Got output buffer %zd"
                " offset=%" PRIi32 " size=%" PRIi32 " ts=%" PRIi64
                " flags=%" PRIu32 "\n", index, info.offset, info.size,
                info.presentationTimeUs, info.flags);

        if (info.flags & ff_AMediaCodec_getBufferFlagPartialFrame(codec)) {
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive got hahaha BufferFlagPartialFrame");
        }

        if (info.flags & ff_AMediaCodec_getBufferFlagMuxerData(codec)) {
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive got hahaha BufferFlagMuxerData");
        }
        ...
        if (info.size) {
            av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive got data");
            if (s->surface) {
                ...
                if (!data) {
                    av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Failed to get output buffer\n");
                    return AVERROR_EXTERNAL;
                }

                if ((ret = mediacodec_wrap_sw_buffer(avctx, s, data, size, index, &info, frame)) < 0) {
                    av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Failed to wrap MediaCodec buffer\n");
                    return ret;
                }

                av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive got new frame: %d\n", s->output_buffer_count);
                /*if(s->out_yuv_seq < 2) {
                    sprintf(filename, "/data/local/tmp/test/%s%d.yuv","mdec", s->out_yuv_seq);
                    yuvf = fopen(filename, "wb+");
                    if(!yuvf) {
                        av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Failed to create file\n");
                    }
                    fwrite(data, sizeof(uint8_t), info.size, yuvf);
                    fclose(yuvf);

                    sprintf(filename, "/data/local/tmp/test/%s%d.yuv","frame", s->out_yuv_seq);
            ...
            if (status < 0) {
                av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Failed to release output buffer\n");
            }
        }

    } else if (ff_AMediaCodec_infoOutputFormatChanged(codec, index)) {
        av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Got OutputFormatChanged");
        char *format = NULL;
        ...
        av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Got OutputBuffersChanged");
        ff_AMediaCodec_cleanOutputBuffers(codec);
    } else if (ff_AMediaCodec_infoTryAgainLater(codec, index)) {
        av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Got TryAgainLater");
        if (s->draining) {
        ...
        av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive Failed to dequeue output buffer (status=%zd)\n", index);
        return AVERROR_EXTERNAL;
    }

    av_log(avctx, AV_LOG_ERROR, "XXXXXXXXXX ff_mediacodec_dec_receive EAGAIN");
    return AVERROR(EAGAIN);
}
...
//-----------------------------------encoder spliter-------------------------------------
static int mediacodec_enc_parse_format(AVCodecContext *avctx, MediaCodecDecContext *s)
{
    int ret = 0;
    int width = 0;
    int height = 0;
...
int ff_mediacodec_enc_init(AVCodecContext *avctx, MediaCodecDecContext *s,
                           const char *mime, FFAMediaFormat *format, int profile)
{
    int ret = 0;
    int status;
    ...
    s->codec = ff_AMediaCodec_createCodecByName(s->codec_name);
    if (!s->codec) {
        av_log(avctx, AV_LOG_ERROR, "Failed to create media encoder for type %s and name %s\n", mime, s->codec_name);
        ret = AVERROR_EXTERNAL;
        goto fail;
    }
...
int ff_mediacodec_enc_send(AVCodecContext *avctx, MediaCodecDecContext *s, AVFrame *frame, AVPacket *pkt)
{
    av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send start");
    int offset = 0;
    int need_draining = 0;
    uint8_t *data;
    ssize_t index = s->current_input_buffer;
    ...
    if (s->flushing) {
        av_log(avctx, AV_LOG_ERROR, "Encoder is flushing and cannot accept new buffer "
                                    "until all output buffers have been released\n");
        return AVERROR_EXTERNAL;
    }
    ...
    if (!frame) {
        ...
        switch (frame->format) {
            ...
            case COLOR_QCOM_FormatYUV420SemiPlanar32m:
                av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send color format OK: %d", frame->format);
                break;
            default:
                av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send color format not supported: %d", frame->format);
                return AVERROR_EXTERNAL;
        }

        pts = frame->pts;
        w = s->crop_right - s->crop_left;
        h = s->crop_bottom - s->crop_top;
        if(!w) {
            w = s->width;
        }
        if(!h) {
        ...
        w2 = FFALIGN(w, 2);
        // get data from AVFrame
        frame_size = h * w + h / 2 * w2;
        // frame_data = av_mallocz(frame_size);
        frame_data = malloc(frame_size);
        if(!frame_data) {
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send alloc frame data error");
            return AVERROR(EINVAL);
        }
        ...
        for(i = 0; i< h; i++)
        {
            memcpy(frame_data + offset, frame->data[0] + i * linesize, w);
            offset += w;
        }
        linesize = frame->linesize[1];
        for(i = 0; i < h / 2; i++)
        {
            memcpy(frame_data + offset, frame->data[1] + i * linesize, w2);
            // convert nv12 to nv21
            for(j = 0; j < w2; j+=2) {
                tmp = frame_data[offset + j];
                frame_data[offset + j] = frame_data[offset + j + 1];
                frame_data[offset + j + 1] = tmp;
            }
            offset += w2;
        }
    }
    ...
    while (offset < frame_size || (need_draining && !s->draining)) {
        index = ff_AMediaCodec_dequeueInputBuffer(codec, input_dequeue_timeout_us);
        if (ff_AMediaCodec_infoTryAgainLater(codec, index)) {
            get_input_buffer_retry++;
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send No input buffer available, try again later: %d\n", get_input_buffer_retry);
            if(get_input_buffer_retry > 3) {
                ff_mediacodec_enc_receive(avctx, s, pkt);
                get_input_buffer_retry = 0;
                continue;
            } else {
                sleep(10);
        ...
        get_input_buffer_retry = 0;
        if (index < 0) {
            // here we need try to get output to free new input buffer
            if(pkt->size > 0) {
                av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send Failed to dequeue input buffer (status=%zd)\n", index);
                break;
            } else {
                av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send Failed to dequeue input buffer (status=%zd), try to get output to free input buffer\n", index);
                ff_mediacodec_enc_receive(avctx, s, pkt);
                continue;
            }
        }
        s->current_input_buffer = -1; // reset current input buffer index

        data = ff_AMediaCodec_getInputBuffer(codec, index, &size);
        if (!data) {
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send Failed to get input buffer\n");
            ret = AVERROR_EXTERNAL;
            goto done;
        }
        ...
        if (need_draining) {
            ...
            av_log(avctx, AV_LOG_DEBUG, "YYYYYYYYYY ff_mediacodec_enc_send Sending End Of Stream signal\n");

            status = ff_AMediaCodec_queueInputBuffer(codec, index, 0, 0, pts, flags);
            if (status < 0) {
                av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send Failed to queue input empty buffer (status = %d)\n", status);
                ret = AVERROR_EXTERNAL;
                goto done;
            }

            av_log(avctx, AV_LOG_ERROR,
                   "YYYYYYYYYY ff_mediacodec_enc_send Queued empty EOS input buffer %zd with flags=%d\n", index, flags);
        ...
        memcpy(data, frame_data + offset, size);
        offset += size;

        status = ff_AMediaCodec_queueInputBuffer(codec, index, 0, size, pts, 0);
        if (status < 0) {
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send Failed to queue input buffer (status = %d)\n", status);
            ret = AVERROR_EXTERNAL;
            goto done;
        }

        av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_send Queued input buffer %zd size=%zd ts=%"PRIi64"\n", index, size, pts);
    }
...
int ff_mediacodec_enc_receive(AVCodecContext *avctx,
                              ...
                              AVPacket *pkt)
{
    av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive start");
    ...
    if (s->draining) {
        /* If the codec is flushing or need to be flushed, block for a fair
         * amount of time to ensure we got a packet */
        // output_dequeue_timeout_us = OUTPUT_DEQUEUE_BLOCK_TIMEOUT_US;
        output_dequeue_timeout_us = OUTPUT_DEQUEUE_TIMEOUT_US;
    }
    index = ff_AMediaCodec_dequeueOutputBuffer(codec, &info, output_dequeue_timeout_us);
    if (index >= 0) {
        av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY Got output buffer %zd"
                                    " offset=%" PRIi32 " size=%" PRIi32 " ts=%" PRIi64
        " flags=%" PRIu32 "\n", index, info.offset, info.size,
                info.presentationTimeUs, info.flags);
        ...
        if (info.flags & ff_AMediaCodec_getBufferFlagPartialFrame(codec)) {
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive got hahaha BufferFlagPartialFrame");
        }

        if (info.flags & ff_AMediaCodec_getBufferFlagMuxerData(codec)) {
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive got hahaha BufferFlagMuxerData");
        }

        if (info.size) {
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive got data");
            data = ff_AMediaCodec_getOutputBuffer(codec, index, &size);
            if (!data) {
                av_log(avctx, AV_LOG_ERROR, "Failed to get output buffer\n");
                return AVERROR_EXTERNAL;
            }
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY buffer info size:%d  byte buffer size:%d\n", info.size, size);

            if (is_config_data) {
                avctx->extradata_size = info.size;
                avctx->extradata = av_mallocz(info.size + AV_INPUT_BUFFER_PADDING_SIZE);
                if (!avctx->extradata) {
                    av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive alloc memory for extradata error\n");
                    return AVERROR(ENOMEM);
                }
                memcpy(avctx->extradata, data, info.size);
                av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive got config data: %d - %d\n", info.size, size);
            }
            if (key_frame) {
                ret = av_new_packet(pkt, info.size + avctx->extradata_size);
                if (ret < 0) {
                    return AVERROR(ENOMEM);
                }
                pkt->pts = info.presentationTimeUs;
                pkt->flags |= AV_PKT_FLAG_KEY;
                memcpy(pkt->data, avctx->extradata, avctx->extradata_size);
                memcpy(pkt->data + avctx->extradata_size, data, info.size);
            } else {
                ret = av_new_packet(pkt, info.size);
                if (ret < 0) {
                    return AVERROR(ENOMEM);
                }
                pkt->pts = info.presentationTimeUs;
            ...
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive got packet: size:%d, key_frame:%d,pts:%ld", pkt->size, key_frame, pkt->pts);
            ret = 0;
            s->output_buffer_count++;

            status = ff_AMediaCodec_releaseOutputBuffer(codec, index, 0);
            if (status < 0) {
                av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive Failed to release output buffer\n");
            }
            return ret;
        } else {
            status = ff_AMediaCodec_releaseOutputBuffer(codec, index, 0);
        ...
        av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive output buffer changed");
        ff_AMediaCodec_cleanOutputBuffers(codec);
    } else if (ff_AMediaCodec_infoTryAgainLater(codec, index)) {
        if (s->draining) {
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive Failed to dequeue output buffer within %" PRIi64 "ms "
                                                                                          "while draining remaining frames, output will probably lack frames\n",
                    output_dequeue_timeout_us / 1000);
        } else {
            av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive No output buffer available, try again later\n");
        }
    } else {
        av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive Failed to dequeue output buffer (status=%zd)\n", index);
        return AVERROR_EXTERNAL;
    }

    av_log(avctx, AV_LOG_ERROR, "YYYYYYYYYY ff_mediacodec_enc_receive EAGAIN");
    return AVERROR(EAGAIN);
}
